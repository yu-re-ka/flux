...
fun yyAction2 (strm, lastMatch : yymatch) = (yystrm := strm; (DIV))
fun yyAction3 (strm, lastMatch : yymatch) = (yystrm := strm; (EOS))
fun yyAction4 (strm, lastMatch : yymatch) = (yystrm := strm; (LPAREN))
fun yyAction5 (strm, lastMatch : yymatch) = let
      val yytext = yymktext(strm)
      in
        yystrm := strm;
        (NUM (revfold (fn(a,r)=>ord(a)-ord("0")+10*r) (explode yytext) 0))
      end
fun yyAction6 (strm, lastMatch : yymatch) = (yystrm := strm; (RPAREN))
fun yyAction7 (strm, lastMatch : yymatch) = (yystrm := strm; (PLUS))
fun yyAction8 (strm, lastMatch : yymatch) = let
      val yytext = yymktext(strm)
      in
        yystrm := strm; (if yytext="print" then PRINT else ID yytext)
      end
fun yyAction9 (strm, lastMatch : yymatch) = (yystrm := strm; (SUB))
fun yyAction10 (strm, lastMatch : yymatch) = (yystrm := strm; (TIMES))
fun yyAction11 (strm, lastMatch : yymatch) = let
      val yytext = yymktext(strm)
      in
        yystrm := strm; (error ("calc: ignoring bad character "^yytext); lex())
      end
