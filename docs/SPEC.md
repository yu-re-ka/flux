...
Attributes may be defined before the package clause whether it exists or not.
These attributes apply to the entire package.
...
A statement that defines a test case.
    ...
    TestcaseStatement = [ Attributes ] "testcase" identifier [ TestcaseExtention ] Block .
...
The edition of the current script is specified as an `edition` attribute on the package.
It is an error for multiple files within a package or module specify differing editions.

Examples:

Specify the edition and explicit package clause
...
Specify the edition without the need for a package clause
